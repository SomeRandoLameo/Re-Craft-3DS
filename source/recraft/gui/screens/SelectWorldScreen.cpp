...
void SelectWorldBotScreen::Render(int mouseX, int mouseY, float delta) {
  ...
  int maximumSize = Gui::CharHeight * 2 * m_worlds.size();
  ...
  for (size_t i = 0; i < m_worlds.size(); i++) {
    ...
    int y =
        i * (Gui::CharHeight + Gui::CharHeight) + 10 + m_scroll;
    if (m_selectedWorld == (int)i) {
      Amy::Color borderColor = Amy::Color(20, 20, 20, 255);

      // FIX PLS
      RenderData->DrawRect(Amy::fvec2(20, y + 7),
                           Amy::fvec2(150, y + CHAR_HEIGHT + 3), borderColor, 1);
    ...
    if (Gui::EnteredCursorInside(10, y - 3, 140, Gui::CharHeight + 6) &&
        y < 32 * 2) {
...
void SelectWorldBotScreen::ButtonClicked() {
  ...
  if (m_clicked_play && m_selectedWorld != -1) {
    ...
    m_ReCraftCore->InitSinglePlayer(m_out_worldpath, m_out_name, &m_worldType,
                                    Gamemode::Gamemode_Count, false);
  }
...
void SelectWorldBotScreen::LoadLevelSource() {
  ...
  while ((entry = readdir(directory))) {
    ...
    if (access(buffer, F_OK) != -1) {
      ...
      mpack_node_copy_utf8_cstr(mpack_node_map_cstr(root, "name"), name,
                                World::NameSize);
