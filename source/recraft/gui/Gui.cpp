...
bool Gui::Button(float size, const char* label, ...) {
    ...
    SpriteBatch_PushQuad(x + SliceSize, y, -3, middlePieceSize, 20, SliceSize, 46 + (pressed * BUTTON_HEIGHT * 2),
                         middlePieceSize, 20);
    SpriteBatch_PushQuad(x + SliceSize + middlePieceSize, y, -3, SliceSize, 20, 192, 46 + (pressed * BUTTON_HEIGHT * 2),
                         SliceSize, 20);
    ...
    // RenderData->DrawRectFilled(Amy::fvec2(x + SLICE_SIZE, y), Amy::fvec2(middlePieceSize, 20), Amy::Color(255, 255,
    // 255, 255)); RenderData->DrawRectFilled(Amy::fvec2(x + SLICE_SIZE + middlePieceSize, y), Amy::fvec2(SLICE_SIZE,
    // 20), Amy::Color(255, 255, 255, 255));


    SpriteBatch_PushTextVargs(x + (w / 2 - textWidth / 2), y + (BUTTON_HEIGHT - CHAR_HEIGHT) / 2, -1,
                              SHADER_RGB(31, 31, 31), true, INT_MAX, NULL, label, vl);
    va_end(vl);
