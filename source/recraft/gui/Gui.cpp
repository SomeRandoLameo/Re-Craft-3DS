...
int Gui::relativeToAbsoluteSize(float s) const {
...
// TODO: Fix if needed
void Gui::Label(float size, bool shadow, Amy::Color color, bool center,
                const char* text, ...) {
  int wrap = size <= 0.f ? (currentRow.width - relativeX - paddingX)
                         : relativeToAbsoluteSize(size);
  int yTextSize = 0;
  ...
  DrawString(text, windowX + relativeX + xOffset, windowY + relativeY,
             Amy::Color(255, 255, 255, 255));
  int xTextSize =
      0;  // SpriteBatch_PushTextVargs(windowX + relativeX + xOffset, windowY +
...
bool Gui::Button(float size, const char* label, ...) {
  ...
  DrawTexturedModalRect(x + SliceSize, y, SliceSize, baseTextureY,
                        middlePieceSize, BUTTON_HEIGHT);
  DrawTexturedModalRect(x + SliceSize + middlePieceSize, y, 192, baseTextureY,
                        SliceSize, BUTTON_HEIGHT);

  Amy::Color textColor =
      pressed ? Amy::Color(255, 255, 160, 255) : Amy::Color(224, 224, 224, 255);
  DrawCenteredString(label, x + (w * 0.5), y + (BUTTON_HEIGHT * 0.5),
                     textColor);
  ...
  return Input::isKeyUp(KEY_TOUCH) &&
         Gui::WasCursorInside(x, y, w, BUTTON_HEIGHT);
}
...
bool Gui::IsCursorInside(int x, int y, int w, int h) {
  ...
  return sclInputX != 0 && sclInputY != 0 && sclInputX >= x &&
         sclInputX < x + w && sclInputY >= y && sclInputY < y + h;
}
bool Gui::WasCursorInside(int x, int y, int w, int h) {
  ...
  return sclOldInputX != 0 && sclOldInputY != 0 && sclOldInputX >= x &&
         sclOldInputX < x + w && sclOldInputY >= y && sclOldInputY < y + h;
}
bool Gui::EnteredCursorInside(int x, int y, int w, int h) {
  int sclOldInputX = Input::getTouchPrev().px / 2;  // m_guiScale;
  int sclOldInputY = Input::getTouchPrev().py / 2;  // m_guiScale;

  return (sclOldInputX == 0 && sclOldInputY == 0) &&
         Gui::IsCursorInside(x, y, w, h);
}
void Gui::GetCursorMovement(int* x, int* y) {
  ...
  *x = Input::getTouch().px / 2 /*m_guiScale*/ -
       Input::getTouchPrev().px / 2 /*m_guiScale*/;
  *y = Input::getTouch().px / 2 /*m_guiScale*/ -
       Input::getTouchPrev().py / 2 /*m_guiScale*/;
}

void Gui::DrawTexturedModalRect(int x, int y, int textureX, int textureY,
                                int width, int height) const {
  RenderData->DrawFromAtlas(
      Amy::ivec2(x * 2, y * 2), Amy::ivec2(width * 2, height * 2),
...
// here.
void Gui::DrawString(const std::string& text, int x, int y,
                     const Amy::Color& color) const {
...
void Gui::DrawStringWithShadow(const std::string& text, int x, int y,
                               const Amy::Color& color) const {
  DrawString(text, (x + 1), (y + 1), fancyFontShadow(color));
  DrawString(text, x, y, color);
}

void Gui::DrawCenteredString(const std::string& text, int x, int y,
                             const Amy::Color& color, bool shadow) const {
  if (shadow) {
    RenderData->DrawTextEx(Amy::ivec2((x + 1) * 2, (y + 1) * 2), text,
                           fancyFontShadow(color), AmyTextFlags_AlignMid);
  }
  RenderData->DrawTextEx(Amy::fvec2(x * 2, y * 2), text, color,
 ...
 */
void Gui::DrawHorizontalLine(int startX, int endX, int y,
                             const Amy::Color& color) const {
  if (endX < startX) {
    int i = startX;
    startX = endX;
  ...
  RenderData->DrawLine(Amy::ivec2(startX * 2, y * 2),
                       Amy::ivec2((endX + 1) * 2, y * 2), 2, color);
}

/**
 * Draws a 1 pixel (2 real pixel) wide vertical line. Args : x, y1, y2, color
 */
void Gui::DrawVerticalLine(int x, int startY, int endY,
                           const Amy::Color& color) const {
  if (endY < startY) {
    int i = startY;
    startY = endY;
  ...
  RenderData->DrawLine(Amy::ivec2(x * 2, startY * 2),
                       Amy::ivec2(x * 2, (endY + 1) * 2), 2, color);
}

/**
 * Draws a rectangle with the specified coordinates and color.
 */
void Gui::DrawRect(int left, int top, int right, int bottom,
                   const Amy::Color& color) const {
  RenderData->DrawRectFilled(Amy::fvec2(left, top), Amy::fvec2(right, bottom),
                             color);
}
...
#define UV_PRECISION 32768
  ...
  if (!pBlockTex) {
    ...
    pBlockTex->Load((C3D_Tex*)BlockRegistry::GetTextureMap(),
                    TextureMap::MapSize, Amy::fvec4(0, 1, 1, 0));
  }
  ...
  for (int i = 0; i < 6; i++) {
    ...
    for (int j = 0; j < 5; j++) {
      int k = i * 6 + j;
      Amy::fvec3 p = Amy::fvec3(vtx[k].xyz[0] - 0.5f, vtx[k].xyz[1] - 0.5f,
                                vtx[k].xyz[2] - 0.5f);
    ...
    Amy::Rect uvs = Amy::fvec4(
        (float)iconUV[0] / (float)UV_PRECISION,
        (float)(iconUV[1] + TextureMap::TileSize) / (float)UV_PRECISION,
        (float)(iconUV[0] + TextureMap::TileSize) / (float)UV_PRECISION,
        (float)iconUV[1] / (float)UV_PRECISION);
