...
void BlockEvent_RandomTick(World* world, Chunk* chunk, int x[], int y[], int z[]) {
	...
	for (int i = 0; i < CLUSTER_PER_CHUNK; i++) {
		for (int j = 0; j < RANDOMTICKS_PER_CLUSTER; j++) {
            ...
            Block block = Chunk_GetBlock(chunk, defaultPosition);
            ...
            switch (block) {
				case Block_Dirt:
					if (!Block_Opaque(Chunk_GetBlock(chunk, blockPosition),Chunk_GetMetadata(chunk, blockPosition))) {
						Chunk_SetBlock(chunk, defaultPosition, Block_Grass);
					}
					break;
				case Block_Grass:
					if (Block_Opaque(Chunk_GetBlock(chunk,blockPosition), Chunk_GetMetadata(chunk, blockPosition))) {
						Chunk_SetBlock(chunk, defaultPosition, Block_Dirt);
					}
					break;
