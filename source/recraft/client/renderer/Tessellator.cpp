...
Tessellator* Tessellator::getInstance() { return s_instance; }

Tessellator* Tessellator::getUniqueInstance(int n) { return new Tessellator(n); }

void Tessellator::createInstance(int n) {
    if (!s_instance)
        s_instance = new Tessellator(n);
}
...
Tessellator::Tessellator(int maxVerts) :
    count(0), _bufCap(maxVerts), _bufPos(0), _quadVerts(0), _u(0), _v(0), _col(0xFFFFFFFF), _hasColor(false),
    _hasTexture(false), _noColor(false), _anyColor(false), _mode(TESS_QUADS), _xo(0), _yo(0), _zo(0) {
    bounds.reset();
...
void Tessellator::begin() { begin(TESS_QUADS); }
...
void Tessellator::flush() {
    ...
    C3D::BufCfg<4>(_buf, sizeof(TessVertex));
...
void Tessellator::color(float r, float g, float b) { color((int)(r * 255), (int)(g * 255), (int)(b * 255), 255); }

void Tessellator::color(float r, float g, float b, float a) {
    color((int)(r * 255), (int)(g * 255), (int)(b * 255), (int)(a * 255));
}

void Tessellator::color(int r, int g, int b) { color(r, g, b, 255); }
...
void Tessellator::color(int p) { color((p >> 16) & 0xFF, (p >> 8) & 0xFF, p & 0xFF, 255); }

void Tessellator::color(int p, int a) { color((p >> 16) & 0xFF, (p >> 8) & 0xFF, p & 0xFF, a); }

void Tessellator::noColor() { _noColor = true; }
    ...
    if (_mode == TESS_QUADS) {
        ...
        if (_quadVerts == 4) {
            if (_bufPos + 6 <= _bufCap) {
                _buf[_bufPos++] = _quad[0];
                _buf[_bufPos++] = _quad[1];
                _buf[_bufPos++] = _quad[2];
                _buf[_bufPos++] = _quad[0];
                _buf[_bufPos++] = _quad[2];
                _buf[_bufPos++] = _quad[3];
            }
            _quadVerts = 0;
        }
    } else {
...
bool Tessellator::hasMaxVertices() const { return (_bufPos * (int)sizeof(TessVertex)) > 60 * 1024; }
