...
void Cursor::Draw(int projUniform, C3D_Mtx* projectionview, World* world, mc::Vector3i pos, Direction highlight) {
    // Create model matrix
    C3D_Mtx model;
    Mtx_Identity(&model);
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (world->GetBlockID(mc::Vector3i(pos.x + offset[0], pos.y + offset[1], pos.z + offset[2])) == BlockID::Air) {
