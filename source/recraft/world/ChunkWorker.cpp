...
ChunkWorker::ChunkWorker() {
    ...
    m_thread = threadCreate(&ChunkWorker::MainloopWrapper, (void*)this, CHUNKWORKER_THREAD_STACKSIZE, prio - 1,
                            isNew3ds ? 2 : 1, false);
...
void ChunkWorker::AddHandler(WorkerItemType type, WorkerFuncObj obj) { m_handler[type].push_back(obj); }
...
void ChunkWorker::Mainloop() {
    ...
    while (!m_shouldStop || m_queue.queue.size() > 0) {
        ...
        privateQueue.insert(privateQueue.end(), m_queue.queue.begin(), m_queue.queue.end());
        m_queue.queue.clear();
