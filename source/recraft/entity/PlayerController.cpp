...
static void convertPlatformInput(InputData *input, float ctrls[],
  ...
  const struct {
    int index;
    u32 key;
  } binaryKeys[] = {{K3DS_A, KEY_A},         {K3DS_B, KEY_B},
  ...
  const struct {
    ...
    float value;
    u32 key;
  } circlePadKeys[] = {{K3DS_CPAD_UP, circY, KEY_CPAD_UP},
  ...
  for (int i = 0; i < 4; i++) {
    ...
    ctrls[idx] =
        ABS(val) * (float)((input->keysdown & key) || (input->keysheld & key));
    keysdown[idx] = input->keysdown & key;
    keysup[idx] = input->keysup & key;
  }
  ...
  const struct {
    ...
    float value;
    u32 key;
  } cstickKeys[] = {{K3DS_CSTICK_UP, cstickY, KEY_CSTICK_UP},
...
PlayerControlScheme::PlayerControlScheme()
    : forward(0), backward(0), strafeLeft(0), strafeRight(0), lookLeft(0),
      lookRight(0), lookUp(0), lookDown(0), placeBlock(0), breakBlock(0),
      jump(0), switchBlockLeft(0), switchBlockRight(0), openCmd(0), crouch(0) {}
...
PlayerController::PlayerController(Player *player)
  ...
  if (access(path, F_OK) != -1) {
    ...
    int *controlSchemeKeys[] = {&controlScheme.forward,
                                &controlScheme.backward,
                                &controlScheme.strafeLeft,
                                &controlScheme.strafeRight,
                                &controlScheme.lookLeft,
                                &controlScheme.lookRight,
                                &controlScheme.lookUp,
                                &controlScheme.lookDown,
                                &controlScheme.placeBlock,
                                &controlScheme.breakBlock,
                                &controlScheme.jump,
                                &controlScheme.switchBlockLeft,
                                &controlScheme.switchBlockRight,
                                &controlScheme.openCmd,
    ...
    if (!ini_sget(cfg, "controls", "auto_jumping", "%d",
                  &this->player->autoJumpEnabled))
      elementMissing = true;
...
void PlayerController::Update(DebugUI *debugUi, InputData input, float dt) {
  ...
  convertPlatformInput(&input, agnosticInput.keys, agnosticInput.keysdown,
                       agnosticInput.keysup);
  ...
  if (Vector3d_MagSqr(movement) > 0.f) {
    float speed =
        4.3f * Vector3d_mag(mc::Vector3d(-strafeLeft + strafeRight,
                                         -crouch + jump, -forward + backward));
  ...
  bool switchBlockLeft =
      WasKeyPressed(controlScheme.switchBlockLeft, &agnosticInput);
  bool switchBlockRight =
      WasKeyPressed(controlScheme.switchBlockRight, &agnosticInput);
