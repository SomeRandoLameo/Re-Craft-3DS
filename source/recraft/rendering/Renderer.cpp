...
Renderer::Renderer(World *world_, Player *player_, WorkQueue *queue) {
  ...
  renderTargets[0] = Top[0]->Ptr();
  renderTargets[1] = Top[1]->Ptr();
  lowerScreen = Bottom->Ptr();
  ...
  world_shader_uLocProjection =
      shaderInstanceGetUniformLocation(world_shader.vertexShader, "projection");
  ...
  gui_shader_uLocProjection =
      shaderInstanceGetUniformLocation(gui_shader.vertexShader, "projection");
  ...
  worldRenderer =
      new WorldRenderer(player, world, workqueue, world_shader_uLocProjection);
...
void Renderer::RenderFrame(int eyeIndex, float iod) {
  C3D_RenderTargetClear(renderTargets[eyeIndex], C3D_CLEAR_ALL, CLEAR_COLOR_SKY,
                        0);
...
void Renderer::RenderLowerScreen(DebugUI *debugUi) {
  ...
  if (*ReCraftCore::GetInstance()->GetGameState() == GameState_SelectWorld) {
    ...
    Inventory::renderHotbar(160 / 2 - 194 / 2,
                            ...
                            player->quickSelectBar, player->quickSelectBarSlot);

    player->inventorySite = Inventory::draw(
        ((137 / 2) - (120 / 2)), 10, player->inventory,
        sizeof(player->inventory) / sizeof(ItemStack), player->inventorySite);
