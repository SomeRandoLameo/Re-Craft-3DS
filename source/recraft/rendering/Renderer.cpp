...
void Renderer::RenderFrame(int eyeIndex, float iod) {
    ...
    if (ReCraftCore::GetInstance()
            ->m_pTopScreen) { /*  TODO: 0 is enough for now, but later we need   1000.f / Amy::App::Delta()*/
        ReCraftCore::GetInstance()->m_pTopScreen->Render3D(0, 0, eyeIndex, m_world_shader_uLocProjection, iod, 0);
        ReCraftCore::GetInstance()->m_pTopScreen->Render(0, 0, 0);
    }
...
void Renderer::RenderLowerScreen(DebugUI* debugUi) {
    ...
    if (ReCraftCore::GetInstance()->m_pBotScreen) {
        ...
        ReCraftCore::GetInstance()->m_pBotScreen->Render(m_mouseX, m_mouseY, 0);
    } /*  TODO: 0 is enough for now, but later we need   1000.f / Amy::App::Delta()*/
