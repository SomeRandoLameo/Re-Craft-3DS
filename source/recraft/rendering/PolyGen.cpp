...
//TODO: I added a bug somewhere, needs resolving
...
Block fastBlockFetch(World* world, ChunkColumnPtr column, ChunkPtr chunk, mc::Vector3i blockPos) {
	return (blockPos.x < 0 || blockPos.y < 0 || blockPos.z < 0 || blockPos.x >= Chunk::Size || blockPos.y >= Chunk::Size || blockPos.z >= Chunk::Size)
		   ? world->GetBlock(mc::Vector3i(
                   (column->x * Chunk::Size) + blockPos.x,
                   (chunk->y * Chunk::Size) + blockPos.y,
                   (column->z * Chunk::Size) + blockPos.z)
                   )
		   : chunk->GetBlock(blockPos.x, blockPos.y, blockPos.z);
}
...
uint8_t fastMetadataFetch(World* world, ChunkColumnPtr column, ChunkPtr chunk, mc::Vector3i blockPos) {
	return (blockPos.x < 0 || blockPos.y < 0 || blockPos.z < 0 || blockPos.x >= Chunk::Size || blockPos.y >= Chunk::Size || blockPos.z >= Chunk::Size)
		   ...
		   : (chunk->metadataLight[blockPos.x][blockPos.y][blockPos.z] & 0xf);
}
...
void addFace(mc::Vector3i facePos, Direction dir, Block block, uint8_t metadata, int ao, bool transparent) {
	if (facePos.x >= 0 && facePos.y >= 0 && facePos.z >= 0 && facePos.x < Chunk::Size && facePos.y < Chunk::Size && facePos.z < Chunk::Size) {
		faceBuffer[currentFace++] = Face{static_cast<int8_t>(facePos.x), static_cast<int8_t>(facePos.y), static_cast<int8_t>(facePos.z), dir, block, static_cast<int8_t>(ao), metadata, transparent};
...
uint16_t floodFill(Chunk* cluster, mc::Vector3i pos, Direction entrySide0, Direction entrySide1, Direction entrySide2) {
	if (floodfill_visited[pos.x][pos.y][pos.z] & 1) return 0;
    ...
    floodfill_queue.clear();
	floodfill_queue.push_back(QueueElement{static_cast<int8_t>(pos.x), static_cast<int8_t>(pos.y), static_cast<int8_t>(pos.z)});
	...
	while (floodfill_queue.size() > 0) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            mc::Vector3i itemPos(item.x + offset[0],item.y + offset[1],item.z + offset[2]);

			if (itemPos.x < 0 || itemPos.y < 0 || itemPos.z < 0 || itemPos.x >= Chunk::Size || itemPos.y >= Chunk::Size || itemPos.z >= Chunk::Size) {
				exitPoints[i] = true;
			} else {
				if (!Block_Opaque(cluster->GetBlock(itemPos.x,itemPos.y,itemPos.z), cluster->metadataLight[itemPos.x][itemPos.y][itemPos.z] & 0xf) &&
				    !(floodfill_visited[itemPos.x][itemPos.y][itemPos.z] & 1)) {
					floodfill_visited[itemPos.x][itemPos.y][itemPos.z] |= 1;
					floodfill_queue.push_back((QueueElement){static_cast<int8_t>(itemPos.x), static_cast<int8_t>(itemPos.y), static_cast<int8_t>(itemPos.z)});
				}
				if ((cluster->GetBlock(item.x,item.y,item.z) == Block::Air ||
                     Block_Opaque(cluster->GetBlock(itemPos.x,itemPos.y,itemPos.z), cluster->metadataLight[itemPos.x][itemPos.y][itemPos.z] & 0xf)) &&
                    cluster->GetBlock(itemPos.x,itemPos.y,itemPos.z) != Block::Air) {
					addFace(mc::Vector3i(itemPos.x, itemPos.y, itemPos.z), DirectionOpposite[i], cluster->GetBlock(itemPos.x, itemPos.y, itemPos.z),
						cluster->metadataLight[itemPos.x][itemPos.y][itemPos.z] & 0xf, 0,
						!Block_Opaque(cluster->GetBlock(itemPos.x, itemPos.y, itemPos.z), cluster->metadataLight[itemPos.x][itemPos.y][itemPos.z] & 0xf));
				}
			}
		}
	...
	for (int i = 0; i < ChunkColumn::ChunksPerColumn; i++) {
		...
		if (chunk->revision != chunk->vboRevision || chunk->forceVBOUpdate) {
				...
				for (int z = 0; z < Chunk::Size; z++) {
					...
					for (int y = 0; y < Chunk::Size; y++) {
						...
						if (!Block_Opaque(chunk->GetBlock(x,y,z), chunk->metadataLight[x][y][z] & 0xf)) {
                            visibility |= floodFill(chunk, mc::Vector3i(x, y, z), xDir, yDir, zDir);
                        }

						Block block = fastBlockFetch(world, item.column, chunk, mc::Vector3i(x + (!x ? -1 : 1), y, z));
						uint8_t meta = fastMetadataFetch(world, item.column, chunk, mc::Vector3i(x + (!x ? -1 : 1), y, z));
						if (!Block_Opaque(block, meta) && chunk->GetBlock(x,y,z) != Block::Air) {
							addFace(
                                    mc::Vector3i(x, y, z), xDir, chunk->GetBlock(x,y,z), chunk->metadataLight[x][y][z] & 0xf,
                                    0,
			...
			for (int y = 0; y < Chunk::Size; y += Chunk::Size - 1) {
				...
				for (int x = 0; x < Chunk::Size; x++) {
					...
					for (int z = 0; z < Chunk::Size; z++) {
						...
						Block block = fastBlockFetch(world, item.column, chunk, mc::Vector3i(x, y + (!y ? -1 : 1), z));
						uint8_t meta = fastMetadataFetch(world, item.column, chunk, mc::Vector3i(x, y + (!y ? -1 : 1), z));
						if (!Block_Opaque(block, meta) && chunk->GetBlock(x,y,z) != Block::Air) {
							addFace(
                                    mc::Vector3i(x, y, z), yDir, chunk->GetBlock(x,y,z), chunk->metadataLight[x][y][z] & 0xf,
                                    0,
                                    !Block_Opaque(chunk->GetBlock(x,y,z), chunk->metadataLight[x][y][z] & 0xf));
						}
					}
			...
			for (int z = 0; z < Chunk::Size; z += Chunk::Size - 1) {
				...
				for (int x = 0; x < Chunk::Size; x++) {
					...
					for (int y = 0; y < Chunk::Size; y++) {
						...
						Block block = fastBlockFetch(world, item.column, chunk, mc::Vector3i(x, y, z + (!z ? -1 : 1)));
						if (!Block_Opaque(block, chunk->metadataLight[x][y][z] & 0xf) &&
                            chunk->GetBlock(x,y,z) != Block::Air) {
							addFace(
                                    mc::Vector3i(x, y, z), zDir, chunk->GetBlock(x,y,z), chunk->metadataLight[x][y][z] & 0xf,
                                    0,
                                    !Block_Opaque(chunk->GetBlock(x,y,z), chunk->metadataLight[x][y][z] & 0xf));
						}
					}
			...
			if (WorldToChunkCoord(px) == item.column->x && WorldToChunkCoord(pz) == item.column->z &&
				...
				floodFill(chunk, mc::Vector3i(WorldToLocalCoord(px), WorldToLocalCoord(py), WorldToLocalCoord(pz)),
