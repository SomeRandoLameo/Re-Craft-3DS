...
void Texture_Load(C3D_Tex* result, const char* filename) {
    ...
    if (image != NULL) {
        ...
        if (width < 64 || height < 64) {
            C3D_SyncTextureCopy(imgInLinRam, 0, (u32*)result->data, 0, width * height * sizeof(u32), 8);
        } else {
            C3D_SyncDisplayTransfer(
                (u32*)imgInLinRam, GX_BUFFER_DIM(width, height), (u32*)result->data, GX_BUFFER_DIM(width, height),
...
void Texture_MapInit(Texture_Map* map, const char** files, int num_files) {
    ...
    C3D_SyncDisplayTransfer(buffer, GX_BUFFER_DIM(Texture::MapSize, Texture::MapSize), (u32*)map->texture.data,
                            GX_BUFFER_DIM(Texture::MapSize, Texture::MapSize),
                            (GX_TRANSFER_FLIP_VERT(1) | GX_TRANSFER_OUT_TILED(1) | GX_TRANSFER_RAW_COPY(0) |
