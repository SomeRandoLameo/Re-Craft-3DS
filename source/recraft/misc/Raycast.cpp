...
bool Raycast_Cast(World* world, mc::Vector3f inpos, mc::Vector3f raydir, Raycast_Result* out) {
    mc::Vector3f map = mc::Vector3f(FastFloor(inpos.x), FastFloor(inpos.y), FastFloor(inpos.z));

    mc::Vector3f square = mc::Vector3f(raydir.x * raydir.x, raydir.y * raydir.y, raydir.z * raydir.z);

    mc::Vector3f deltaDist =
        mc::Vector3f(sqrtf(1.f + (square.y + square.z) / square.x), sqrtf(1.f + (square.x + square.z) / square.y),
    ...
    while (hit == 0) {
        ...
        if (world->GetBlockID(ToVector3i(map)) != BlockID::Air || world->GetBlockID(ToVector3i(map)) == BlockID::Lava)
            hit = 1;
