...
Block MCBridge::MCLibBlockToCTBlock(u32 BlockType) {
    ...
    // 96 - 101 = sapling
        ...
        return Block::Water; // Flowing water, actually
    } else if (BlockType == 144) {
        return Block::Water;
    } else if (BlockType == 160) {
        return Block::Lava; // Flowing lava, actually
...
ItemStack MCBridge::MCLIBSlotToCTItemStack(mc::inventory::Slot in) {
    return (ItemStack){static_cast<Block>(in.GetItemId()), (u8)in.GetItemDamage(), in.GetItemCount()};
}
