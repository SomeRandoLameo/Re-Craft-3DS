...
static inline uint16_t BlockIDToItemIndex(BlockID id) {
    return static_cast<uint16_t>(id);
}

static inline uint16_t ItemIDToIndex(ItemID id) {
    return static_cast<uint16_t>(id);
}
...
public:
    ...
    Item* setUnlocalizedName(const std::string& name) { m_unlocalizedName = name; return this; }
    Item* setHasSubtypes(bool has) { m_hasSubtypes = has; return this; }
    Item* setFull3D() { m_bFull3D = true; return this; }
    Item* setMaxDamage(int damage) { m_maxDamage = damage; return this; }
...
public:
    ...
    static Item* GetItem(ItemID id) {
        ...
        int idx = static_cast<int>(id);
        if (idx < 0 || idx >= MaxItems) return nullptr;
        if (!instance.m_occupied[idx]) return nullptr;
