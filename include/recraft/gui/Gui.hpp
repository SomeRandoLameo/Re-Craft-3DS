...
class Gui {
 public:
  constexpr static int SliceSize = 8;
  constexpr static int CharHeight = 8;
  ...
  void Label(float size, bool shadow, Amy::Color color, bool center,
             const char* text, ...);
  ...
  void DrawTexturedModalRect(int x, int y, int textureX, int textureY,
  ...
  void DrawStringWithShadow(const std::string& text, int x, int y,
                            const Amy::Color& color) const;
  void DrawCenteredString(const std::string& text, int x, int y,
                          const Amy::Color& color, bool shadow = true) const;

  void DrawHorizontalLine(int startX, int endX, int y,
                          const Amy::Color& color) const;
  void DrawVerticalLine(int x, int startY, int endY,
                        const Amy::Color& color) const;
 ...
 private:
  ...
  Amy::mat4 pIconMtx;
  Amy::Texture::Ref pBlockTex = nullptr;
};
