...
namespace Input {
    static inline InputManager* getMgr() { return InputManager::getInstance(); }
    static inline bool isPressed(InputAction action) { return getMgr()->actPress(action); }
    static inline bool isReleased(InputAction action) { return getMgr()->actRel(action); }
    static inline bool isKeyDown(u32 key) { return getMgr()->keyDown(key); }
    static inline bool isKeyHeld(u32 key) { return getMgr()->keyHeld(key); }
    static inline bool isKeyUp(u32 key) { return getMgr()->keyUp(key); }
    static inline u32 getKey(InputAction action) { return getMgr()->getKey(action); }
    static inline circlePosition getPad() { return getMgr()->getPad(); }
    static inline circlePosition getStick() { return getMgr()->getStick(); }
    static inline touchPosition getTouch() { return getMgr()->getTouch(); }
    static inline touchPosition getTouchPrev() { return getMgr()->getTouchPrev(); }
} // namespace Input
